...
class LLMBase(ABC):
    ...
    def chat(self, msgs: Union[str, Dict[str,str], List[Dict[str,str]], LLMMessage, List[LLMMessage]], 
             model: str, temperature: float, stream: bool, **kwargs) -> Generator[str, None, None] | str:
        ...
        # 调用底层的 chat_raw 方法
        response = self._chat_raw(messages, model, temperature, stream, **kwargs)
        return response

    @abstractmethod
    def _chat_raw(self, messages: List[Dict[str, str]], model: str, temperature: float, stream: bool, **kwargs) -> Generator[str, None, None] | str:
    ...
    def chat_with_context(self, msgs: Union[str, LLMMessage, List[LLMMessage], Dict[str,str], List[Dict[str,str]]], 
        ...
        msgs = self.__process_messages(msgs)
        ...
        if not stream:
            # 非流式模式：直接返回响应字符串
            n_msg = LLMMessage(role='assistant', content=response)
